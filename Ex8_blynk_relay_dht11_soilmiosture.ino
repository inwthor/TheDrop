...
char ssid[] = "Galaxy A51ED49";                     //ชื่อ SSID ที่เราต้องการเชื่อมต่อ        
char pass[] = "12345678";                         //รหัสผ่าน WI-FI
char auth[] = "luBXHFYDzUHotc-jX7rnVVTuGMbRRcHc"; //Auth token from blynk app 
...
void setup() {
  ...
  timer.setInterval(1000L,dhtSensorData);
  timer.setInterval(1000L, getSoilMoisterData1);
  timer.setInterval(1000L, getSoilMoisterData2);
  timer.setInterval(1000L, getSoilMoisterData3);
  timer.setInterval(1000L, getSoilMoisterData4);  
  timer.setInterval(1000L, clockDisplay);
  timer.setInterval(1000L, reconnectblynk);  //Function reconnect  
...
v_1 = valueValve1;
  if(valueValve1 == 1){
    ...
    Blynk.setProperty(Widget_LED_Pump, "color", "#ff0000");
...
v_2 = valueValve2;
  if(valueValve2 == 1){
    ...
    digitalWrite(pump, HIGH);
    Blynk.virtualWrite(Widget_Btn_Pump,1);
    Blynk.setProperty(Widget_LED_Pump, "color", "#ff00e1");
    Blynk.setProperty(Widget_LED_Pump, "label", "ปั้มน้ำกำลังทำงาน");
    LedBlynkPump.on(); 
    }
    else{              
...
v_3 = valueValve3;
  if(valueValve3 == 1){       
    ...
    digitalWrite(pump, HIGH);
    Blynk.virtualWrite(Widget_Btn_Pump,1);
    Blynk.setProperty(Widget_LED_Pump, "color", "#0051ff");
    Blynk.setProperty(Widget_LED_Pump, "label", "ปั้มน้ำกำลังทำงาน");
    LedBlynkPump.on(); 
    }
    else{              
